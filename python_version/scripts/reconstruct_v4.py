...
Python port of reconstruct_v4.m
"""
...
def tv_phi_replicate(cube_in: np.ndarray) -> float:
    """TV norm with replicate boundary (matches MATLAB code)."""
    cube_in = cube_in.astype(np.float32, copy=False)
    h, w, b = cube_in.shape
    tv_val = 0.0
    for band in range(b):
        u = cube_in[:, :, band]
        u_right = u[:, np.r_[1:w, w - 1]]
        u_down = u[np.r_[1:h, h - 1], :]
        dx = u_right - u
        dy = u_down - u
        tv_val += float(np.sum(np.sqrt(dx * dx + dy * dy)))
    return tv_val
...
def main():
    ...
    wavelength_nm = np.arange(400, 701, 10, dtype=np.float32)
    rgb_gt_path = os.path.join(ROOT_DIR, "..", "feathers_ms", "feathers_RGB.bmp")
    cmf_npz = os.path.join(DATA_DIR, "cie1931_2deg_400_700_10nm.npz")
    rgb_out_png = "pseudoColor_sRGB.png"
    ...
    def psi_tv(cube_in, tau_prox):
        ...
        lambd = 1.0 / tau_prox
        return tv_denoise_chambolle(cube_in, lambd, n_iter=tv_inner_iters)
    ...
    # Note: twist() currently supports initialization=2 (A'*y). For exact
    # x0 control, extend twist() to accept an explicit x0.
    result = twist(
    ...
    cmf = np.load(cmf_npz)
    rgb_recon = hs_to_srgb_cie1931(recon_cube, cmf["wavelength_nm"], cmf["xbar"], cmf["ybar"], cmf["zbar"])
    rgb_hs_gt = hs_to_srgb_cie1931(hs, cmf["wavelength_nm"], cmf["xbar"], cmf["ybar"], cmf["zbar"])
    ...
    print("Saved reconstruction data: HS_recon_TwIST_Integrated.npz")
